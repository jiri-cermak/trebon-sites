...
- **Reference specification:** `/opt/data/projects/redesign/NEW_V7_design_specification.md`
...
**Skill isolation:** Every `delegate_task` call MUST set `workdir="/opt/data"`. The runtime auto-detects project context files from the working directory and injects them as virtual skills. Renaming `AGENTS.md` to `.agents.md` (dotfile) prevents this — the runtime scans only `.hermes.md`, `AGENTS.md`, `CLAUDE.md`, and `.cursorrules`. The relay context is the subagent's sole source of rules.
